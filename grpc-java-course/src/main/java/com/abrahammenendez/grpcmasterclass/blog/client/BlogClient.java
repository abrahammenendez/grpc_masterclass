package com.abrahammenendez.grpcmasterclass.blog.client;
...
public class BlogClient {
  ...
  public static void main(String[] args) {
    System.out.println("Helo I'm a gRPC client for Blog");
  ...
  private void run() {
    ...
    CreateBlogResponse createResponse =
        blogClient.createBlog(CreateBlogRequest.newBuilder().setBlog(blog).build());
    ...
    ReadBlogResponse readBlogResponse =
        blogClient.readBlog(ReadBlogRequest.newBuilder().setBlogId(blogId).build());

    System.out.println(readBlogResponse.toString());

    /*System.out.println("Reading blog with nonexisting id");

    ReadBlogResponse readBlogResponseNotFound =
        blogClient.readBlog(
            ReadBlogRequest.newBuilder().setBlogId("5eae09ddba0e30f7ec47551d").build());*/
    ...
    UpdateBlogResponse updateBlogResponse =
        blogClient.updateBlog(UpdateBlogRequest.newBuilder().setBlog(newBlog).build());
    ...
    DeleteBlogResponse deleteBlogResponse =
        blogClient.deleteBlog(DeleteBlogRequest.newBuilder().setBlogId(blogId).build());
